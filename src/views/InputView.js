...
class InputView {
  ...
  async getInputProductAndQuantity() {
    const productAndQuantityInput = await Console.readLineAsync(
      SYSTEM_MESSAGE.input.requestProductAndQuantity,
    );
  ...
  async getUseMembershipDiscount() {
    const answer = await Console.readLineAsync(
      SYSTEM_MESSAGE.input.requestMembershipDiscount,
    );
    this.#validateAnswer(answer);
  ...
  async getInputRepurchase() {
    const answer = await Console.readLineAsync(
      SYSTEM_MESSAGE.input.requestRepurchase,
    );
    this.#validateAnswer(answer);
