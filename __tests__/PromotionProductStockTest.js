...
describe('PromotionProductStock 테스트', ()=>{
  test('생성자 테스트',()=>{
    ...
    expect(()=>{new PromotionProductStock(productName,quantity,promotionName)}).not.toThrow();
  })
  ...
  test('프로모션 없는 생성자 테스트',()=>{
    ...
    expect(()=>{new PromotionProductStock(productName,quantity,promotionName)}).toThrow('[ERROR]');
  })
