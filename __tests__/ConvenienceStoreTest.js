...
describe('구매할 상품 입력 예외 테스트', () => {
  ...
  test.each([
    ...
    expect(() => convenienceStore.getProductStocksToSell(inputString)).toThrow(
      '[ERROR]',
    );
  });
});
...
describe('ConvenienceStore 재고 감소 테스트', () => {
  ...
  test.each([
    ...
    async ({
      ...
      expectedPromotionProductQuantity,
    }) => {
      // when
      convenienceStore.decrementPromotionProductQuantity(
        isExceed,
        productName,
        decrementQuantity,
      );
      ...
      const updatedPromotionProductQuantity =
        updatedProductInfo.promotionProductQuantity;

      expect(updatedProductQuantity).toBe(expectedProductQuantity);
      expect(updatedPromotionProductQuantity).toBe(
        expectedPromotionProductQuantity,
      );
    },
  );
